...
TEST(ACIPv4ListTestsGroup, ACIPv4List_serialize) {
    ...
    ACIPv4ListWritePacket write_data{
        addresses,
    };
    ...
    auto read_data = ACIPv4ListReadPacket::Deserialize(&raw_data);
...
TEST(ACIPv4ListTestsGroup, Wrapper_ACIPv4List_serialize) {
    uint8_t buffer[256] = {};
    RawData raw_data{ buffer, buffer + sizeof(buffer) };
...
TEST(ACIPv4ListTestsGroup, Wrapper_ACIPv4List_deserialize) {
    // clang-format off
    uint8_t data[] = {
        0x00, 0x02,             // Type = 2 (AC IPv4 List)
        0x00, 0x0C,             // Length = 12 (3 адреса * 4 байта)
