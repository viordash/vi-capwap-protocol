...
TEST_GROUP(AddMacAclEntryTestsGroup){ //
                                      TEST_SETUP(){}

                                      TEST_TEARDOWN(){}
};

TEST(AddMacAclEntryTestsGroup, AddMacAclEntry_serialize) {
    uint8_t buffer[4096] = {};
    ...
    WritableAddMacAclEntry write_data;
    write_data.Add({ mac_6_0 });
    write_data.Add({ mac_8_0 });
    write_data.Add({ mac_8_1 });
    write_data.Add({ mac_6_1 });

    write_data.Serialize(&raw_data);
    CHECK_EQUAL(&buffer[0] + 37, raw_data.current);
    const uint8_t reference[] = { 0x00, 0x07, 0x00, 0x21, 0x04, 0x06, 0x00, 0x1A, 0x2B, 0x3C,
...
TEST(AddMacAclEntryTestsGroup, AddMacAclEntry_deserialize) {
    ...
    uint8_t data[] = {
        ...
        0x00, 0x07,     // Type: 7 (Add MAC ACL Entry)
...
TEST(AddMacAclEntryTestsGroup, Add_array_of_items_is_unique) {
    uint8_t buffer[4096] = {};
    RawData raw_data{ buffer, buffer + sizeof(buffer) };
