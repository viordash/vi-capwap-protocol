...
TEST(OFDMControlTestsGroup, Serialize_Deserialize_array) {
    ...
    w_controls.Add({ 1, 36, 3, 0x01, 100 });
    w_controls.Add({ 2, 40, 4, 0x02, 200 });
    ...
    CHECK_EQUAL(3, r_controls.Get()[0]->GetChannelWidth());
    ...
    CHECK_EQUAL(4, r_controls.Get()[1]->GetChannelWidth());
...
TEST(OFDMControlTestsGroup, Add_array_of_items_is_unique_by_RadioID) {
    ...
    w_controls.Add({ 1, 36, 1, 0x01, 100 });
    w_controls.Add({ 1, 40, 2, 0x03, 150 });  // Replaces first
    w_controls.Add({ 2, 44, 3, 0x02, 200 });
    w_controls.Add({ 2, 48, 4, 0x04, 250 });  // Replaces second
