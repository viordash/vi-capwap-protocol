...
TEST_GROUP(LocationDataTestsGroup){ //
                                    TEST_SETUP(){}

                                    TEST_TEARDOWN(){}
};

TEST(LocationDataTestsGroup, LocationData_serialize) {
    uint8_t buffer[256] = {};
    RawData raw_data{ buffer, buffer + sizeof(buffer) };

    WritableLocationData write_data{ "abcdefабвгд" };
...
TEST(LocationDataTestsGroup, LocationData_deserialize_ascii) {
    // clang-format off
// Юнит-тест: Location data - Пример 1 (простое ASCII-имя)
// Тип элемента = 28, Длина = 14
 uint8_t data[] = {
    // --- TLV Header (4 байта) ---
    0x00, 0x1C,       // Type = 28 (в Big Endian)
...
TEST(LocationDataTestsGroup, LocationData_deserialize_utf8) {
    // clang-format off
// Юнит-тест: Location data - Пример 2 (имя с UTF-8)
// Тип элемента = 28, Длина = 16
uint8_t data[] = {
    // --- TLV Header (4 байта) ---
    0x00, 0x1C,       // Type = 28
    0x00, 0x10,       // Length = 16
